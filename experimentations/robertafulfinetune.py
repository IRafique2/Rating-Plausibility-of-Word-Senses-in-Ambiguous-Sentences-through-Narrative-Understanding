...
def train_model(model_name='roberta-base', epochs=20, batch_size=16, lr=2e-5):
    print("\n" + "="*80)
...
def evaluate(predictions, targets, stdevs, dataset_name='DEV'):
    ...
    print("-" * 70)
    print(f"{'Spearman Correlation':<30} {spearman_corr:<15.4f} {'>0.45':<15} {'' if spearman_corr > 0.45 else ''}")
    print(f"{'Accuracy Within StdDev':<30} {acc:<15.4f} {'>0.70':<15} {'' if acc > 0.70 else ''}")
    print(f"{'RMSE':<30} {rmse:<15.4f} {'<1.10':<15} {'' if rmse < 1.10 else ''}")
    print(f"{'MAE':<30} {mae:<15.4f} {'<0.90':<15} {'' if mae < 0.90 else ''}")
...
print("\n" + "="*80)
print("SEMEVAL 2026 TASK 5: WORD SENSE PLAUSIBILITY RATING (RoBERTa)")
print("="*80)
...
print("\nAnalysis and visualization complete. Plots saved as 'model_results_with_dataset_info.png'.")

import matplotlib.pyplot as plt
import numpy as np

# Ensure uncertainties exist
if 'uncertainties' not in globals():
    uncertainties = dev_df['error'].values  # fallback

# 2x2 plot
fig, axes = plt.subplots(2, 2, figsize=(14, 10), dpi=300)


# 1. Predictions vs Actual Ratings

scatter = axes[0, 0].scatter(
    dev_df['average'].values,
    dev_df['prediction'].values,
    c=uncertainties,
    cmap='viridis',
    alpha=0.6,
    edgecolors='black',
...
axes[0, 0].set_xlabel('Actual Rating', fontsize=12, weight='bold')
axes[0, 0].set_ylabel('Predicted Rating', fontsize=12, weight='bold')
axes[0, 0].set_title('DEV Set: Predictions vs Actual (RoBERTa)', fontsize=14, weight='bold')
axes[0, 0].legend()
...
challenging_mask = dev_df['homonym'].isin(CHALLENGING_HOMONYMS)
challenging_errors = dev_df['error'][challenging_mask]
other_errors = dev_df['error'][~challenging_mask]
...
# 4. Dataset & Results Summary Panel
...
• {len(train_df['homonym'].unique())} unique homonyms
...
• Accuracy within Std: {results['accuracy_within_std']:.4f}
